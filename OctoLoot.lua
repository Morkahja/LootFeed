...
local root = CreateFrame("Frame", "OctoLootFrame", UIParent)
local anchor = CreateFrame("Frame", "OctoLootAnchor", UIParent)
...
local questScan = CreateFrame("GameTooltip", "OctoLootQuestScan", UIParent, "GameTooltipTemplate")
local defaults = { x = -48, y = 100, scale = 1, duration = 6, direction = "down", mute = false }

local function Say(s) DEFAULT_CHAT_FRAME:AddMessage("|cff75d9c5Octo Loot:|r " .. s) end
local function Place()
    anchor:ClearAllPoints()
    anchor:SetScale(OctoLootDB.scale)
    anchor:SetPoint("TOPRIGHT", UIParent, "RIGHT", OctoLootDB.x, OctoLootDB.y)
end
...
local function IsQuestItem(link)
    ...
    for i = 1, questScan:NumLines() do
        local line = getglobal("OctoLootQuestScanTextLeft" .. i)
...
local function MakeRow(index)
    ...
    local row = CreateFrame("Button", "OctoLootRow" .. index, holder)
    row.holder = holder
...
title:SetPoint("CENTER", anchor, "CENTER", 0, 0); title:SetText("Octo Loot - drag here /oloot lock")
...
anchor:SetScript("OnDragStop", function()
    ...
    OctoLootDB.x = this:GetRight()*s - UIParent:GetRight()
    OctoLootDB.y = this:GetTop()*s - UIParent:GetHeight()/2
    Place()
end)
...
root:SetScript("OnEvent", function()
    if event == "ADDON_LOADED" and arg1 == "OctoLoot" then
        if type(OctoLootDB) ~= "table" then OctoLootDB = {} end
        for k, v in pairs(defaults) do if type(OctoLootDB[k]) ~= type(v) then OctoLootDB[k] = v end end
        OctoLootDB.scale = math.max(0.6, math.min(1.8, OctoLootDB.scale))
        OctoLootDB.duration = math.max(2, math.min(20, OctoLootDB.duration))
        ...
        if previous then
            ChatFrame_OnEvent = function(e)
                local ev = e or event
                if ready and OctoLootDB.mute and ev == "CHAT_MSG_LOOT" and Personal(arg1) then return end
                return previous(e)
            end
        end
...
SLASH_OCTOLOOT1 = "/oloot"
SLASH_OCTOLOOT2 = "/octoloot"
SlashCmdList["OCTOLOOT"] = function(message)
    if not ready then return end
    ...
    elseif command == "scale" and tonumber(value) then OctoLootDB.scale = math.max(0.6, math.min(1.8, tonumber(value))); Place()
    elseif command == "duration" and tonumber(value) then OctoLootDB.duration = math.max(2, math.min(20, tonumber(value)))
